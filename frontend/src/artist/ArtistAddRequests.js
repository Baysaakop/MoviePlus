...
function ArtistAddRequests (props) {
    ...
    function getArtists() {
        setLoading(true)
        var url = `${api.tempartists}?artistid=0&page=${page}`          
        axios({
            method: 'GET',
            url: url
        }).then(res => {                                                     
    ...
    return (
        <div style={{ marginTop: '80px', minHeight: '80vh' }}>
            <div style={{ padding: getPadding() }}>           
                <Typography.Title level={3}>Шинэ уран бүтээлчийн хүсэлтүүд</Typography.Title>   
                { loading ? (
...
export default connect(mapStateToProps)(ArtistAddRequests)